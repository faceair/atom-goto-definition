...
export default {
  ...
  activate() {
    ...
    this.subscriptions.add(atom.config.observe('goto-definition.disableScopeNames', (value) => {
      this.disableScopeNames = new Set(value);
    }));
  },
  ...
  getProvider() {
    const avaiableFileExtensions = new Set(Object.keys(Config)
      .map(key => Config[key].files)
    ...
    return {
      ...
      getSuggestionForWord: (editor, text, range) => {
        ...
        if (this.disableScopeNames.has(scopeName)) {
          return null;
        }
